	...
	<tr>
		...
		<td>
			...
			<table border="3" class="responsive-table striped highlight">
				...
				<?php 
					$no=1;
					$pengaduan = mysqli_query($koneksi,"SELECT * FROM pengaduan INNER JOIN customer ON pengaduan.nik=customer.nik INNER JOIN tanggapan ON pengaduan.id_pengaduan=tanggapan.id_pengaduan WHERE pengaduan.nik='".$_SESSION['data']['nik']."' ORDER BY pengaduan.id_pengaduan DESC");
...
if (isset($_POST['kirim'])) {
    ...
    // Periksa apakah ukuran file kurang dari atau sama dengan 2 MB (2 * 1024 * 1024 byte)
    if ($size <= 2097152) {
        if ($foto != "") {
        ...
        echo "<script>alert('Ukuran Gambar Tidak Lebih Dari 2 MB')</script>";
    }
}
?>
